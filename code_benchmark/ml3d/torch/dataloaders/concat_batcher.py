...
class KPConvBatch:
    ...
    def __init__(self, batches):
        ...
        for batch in batches:
            ...
            # 将 batch["attr"]中的数据添加到 self.attr中
            for key in self.attr.keys():
                self.attr[key].append(batch['attr'][key])
        ...
        # Input features (Use reflectance, input height or all coordinates) 输入特征（使用反射率、输入高度或所有坐标）
        ...
        # Extract input tensors from the list of numpy array 从numpy数组的列表中提取输入张量
        ind = 1
        self.points = [
...
class ConcatBatcher(object):
    ...
    def collate_fn(self, batches):
        ...
        if self.model == "KPConv" or self.model == "KPFCNN":
            ...
            return {'data': batching_result, 'attr': batching_result.attr}  # 已经修复
