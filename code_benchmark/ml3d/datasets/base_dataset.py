...
class BaseDatasetSplit(ABC):
    ...
    def __init__(self, dataset, split='training'):
        ...
        data_list, path_list = dataset.get_data_list(path_list)
        ...
        if split in ['test']:
            ...
            sampler_cfg = self.cfg.get('sampler', {'name': 'SemSegRandomSampler'})  # 如果不存在sampler,获取返回值
            # sampler_cls = get_module('sampler', sampler_cfg['name'])

            # sampler_cfg = self.cfg.get('sampler', None)  # 如果不存在sampler,获取返回值
            sampler_cls = None if sampler_cfg is None else get_module('sampler', sampler_cfg['name'])

        # self.sampler = sampler_cls(self)  # init
        self.sampler = None if sampler_cls is None else sampler_cls(self)
