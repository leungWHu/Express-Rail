...
from ml3d.torch import KPFCNN, SemanticSegmentation
...
if __name__ == '__main__':
    ...
    datasetName = FLAGS.data if FLAGS.data is not None else "WhuRailway3D"
    print(f"数据集：{datasetName}, 运行模式：{run_mode}，请注意.... \n")
    time.sleep(3)

    cfg_file = f"dataset/{datasetName}/config_KPFCNN.yaml"  # 当测试模式时，从log文件夹取
    ...
    if run_mode == "train":
        ...
        model = KPFCNN(**cfg.model)  # 创建模型 无需.cuda()，先放在cpu，semantic_segmentation.py会切换到gpu
    ...
    if run_mode == "test":
        ...
        model = KPFCNN(test_ckpt_path=test_ckpt_path, **cfg.model)
